...
public sealed class AudioManager : Singleton
{
    ...
    /// <summary>
    /// /// <summary>
    /// 多通道音效通道数初始值【请勿在代码中修改】
    /// </summary>
    public int maxMultipleAudioCount = 5;
    /// <summary>
    /// 世界音效音量初始值【请勿在代码中修改】
    /// </summary>
    ...
    {
        Debug.Log("source");
        float leastLastPlayTime=100;
        AudioSource leastLastPlayTimeAudioSource=_backgroundAudio;
        for (int i = 0; i < _multipleAudios.Count; i++)
        {
            ...
            {
                if (_multipleAudios[i].clip.length - _multipleAudios[i].time < leastLastPlayTime)
                {
                    leastLastPlayTimeAudioSource = _multipleAudios[i];
                }
            }
        }

        if (_multipleAudios.Count > maxMultipleAudioCount)
        {
            return leastLastPlayTimeAudioSource;
        }
        else
        {
